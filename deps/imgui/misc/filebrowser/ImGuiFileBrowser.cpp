...
bool ImGuiFileBrowser::renderFileListRegion()
{
	...
	for (std::vector<const Info*>::size_type i = 0; i < filtered_dirs.size(); i++)
	{
		if (!filtered_dirs[i]->is_hidden || show_hidden)
		{
			...
			if (ImGui::Selectable(dir_with_icon.c_str(), selected_idx == static_cast<int>(i) && is_dir, ImGuiSelectableFlags_AllowDoubleClick))
			{
				selected_idx = i;
				is_dir = true;
	...
	for (std::vector<const Info*>::size_type i = 0; i < filtered_files.size(); i++)
	{
		if (!filtered_files[i]->is_hidden || show_hidden)
		{
			...
			file_with_icon.append(filtered_files[i]->name);

			if (ImGui::Selectable(file_with_icon.c_str(), selected_idx == static_cast<int>(i) && !is_dir, ImGuiSelectableFlags_AllowDoubleClick))
			{
...
#ifdef OSWIN
	drives_shown = (current_dirlist.size() == 1 && current_dirlist.back() == "PC");
#endif // OSWIN
...
{
	std::filesystem::path dir(pathdir);

	auto valid_dir = std::filesystem::is_directory(dir);
	if (!valid_dir && is_appearing)
	{
		current_dirlist.clear();
		current_path = pathdir = "./";

		dir = std::filesystem::path(pathdir);
		valid_dir = std::filesystem::is_directory(dir);
	}
	...
	if (valid_dir)
	{
		...
		// If we are on Windows and current path is relative then get absolute path
		if (current_dirlist.empty() && pathdir.starts_with("./"))
		{
			std::string current_directory = std::filesystem::absolute(dir).string();
		...
		for (auto& file : dir_iter)
		{
			...
			{
#ifdef OSWIN
				std::string dir = pathdir + filename;
			...
			{
				subdirs.push_back(Info(filename, is_hidden));
			}
			else if (file.is_regular_file() && dialog_mode != DialogMode::SELECT)
			{
				subfiles.push_back(Info(filename, is_hidden));
			}
		}
